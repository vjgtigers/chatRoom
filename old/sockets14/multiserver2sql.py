...
def commands(client, message, userid):
    ...
    if message.startswith("!time"):
        ...
        sqlqueries.roomChange(connection, userid,roomStr[1])
...
def startLeave(message, ids):
    #for client in clients:
    #    client.send(f"{message}".encode("ascii"))
    for i in ids:  # id_socket_client[userid] = user_id
        i = i[0]
        print(id_socket_client[i].socket)
        (id_socket_client[i].socket).send(message.encode("ascii"))
...
def handle(client, PORT, HOST, userid):
    ...
    while True:
        try:
            ...
            ####startLeave(f'{nickname} has left the chat',ids)
...
def receive(HOST, PORT):
    ...
    while True:
        ...
        if e.startswith("VALID") == True:
            ...
            ids = sqlqueries.clientRoomSearch(connection, userid)
            startLeave(f"{nickname} has joined the server", ids)
